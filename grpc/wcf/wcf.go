...
// GetAliasInChatRoom 获取群成员昵称
func (c *Client) GetAliasInChatRoom(roomId string, wxId string) string {
	contacts := c.ExecDBQuery("MicroMsg.db", fmt.Sprintf("SELECT RoomData FROM ChatRoom WHERE ChatRoomName = '%s';", roomId))
	if len(contacts) == 0 || len(contacts[0].GetFields()) == 0 {
		return ""
	}

	var result RoomData
	proto.Unmarshal(contacts[0].GetFields()[0].Content, &result)

	for _, member := range result.GetMembers() {
		if member.GetWxid() == wxId {
			if member.GetName() != "" {
